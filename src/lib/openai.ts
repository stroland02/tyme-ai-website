...
Make it challenging but achievable for their fitness level. Include proper form cues and safety tips.`;
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a certified personal trainer and fitness expert. Provide safe, effective workout plans.',
      },
      {
        role: 'user',
        content: prompt,
...
Make it balanced, nutritious, and aligned with their goals.`;
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a registered dietitian and nutrition expert. Provide balanced, healthy meal plans.',
      },
      {
        role: 'user',
        content: prompt,
...
Keep it encouraging and constructive.`;
  ...
  const completion = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: 'You are a certified personal trainer providing performance analysis.',
      },
      {
        role: 'user',
        content: prompt,
  ...
  return completion.choices[0].message.content || 'Unable to analyze performance at this time.';
}
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are an enthusiastic fitness coach providing daily motivation.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    return (
      ...
      `Amazing work, ${userName}! ${streak} days strong! 🔥`
    );
  } catch (error) {
    console.error('OpenAI motivation generation failed:', error);
